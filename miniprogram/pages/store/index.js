...
function campaignScene(campaignId) {
  return wx.getStorageSync(campaignSceneKey(campaignId)) || "";
}
...
Page({
  ...
  async onShow() {
    const storedSection = wx.getStorageSync("store_section");
    if (storedSection) {
      wx.removeStorageSync("store_section");
      this.setData({ activeSection: storedSection });
    }
    await this.loadProducts();
    if (this.pendingCampaignId) {
      const campaignId = this.pendingCampaignId;
      this.pendingCampaignId = 0;
      await this.viewCampaign({ currentTarget: { dataset: { id: campaignId } } });
    }
    wx.removeStorageSync("store_product_id");
    const storedCampaignId = Number(wx.getStorageSync("store_campaign_id") || 0);
    if (storedCampaignId) {
      wx.removeStorageSync("store_campaign_id");
      await this.viewCampaign({ currentTarget: { dataset: { id: storedCampaignId } } });
    }
  },
  ...
  async loadProducts() {
    try {
      const campaigns = await api.acquisitionCampaigns({ keyword: this.data.keyword });
      this.stopScreenHeartbeat();
      this.setData({
        categories: ["全部"],
        products: [],
      ...
      if (campaigns.length) {
        await this.viewCampaign({ currentTarget: { dataset: { id: campaigns[0].id } } });
      }
    } catch (error) {
  ...
  async buyCampaign(event) {
    try {
      const user = await getApp().ensureUser();
      const campaignId = Number(event.currentTarget.dataset.id);
      let campaign = this.data.selectedCampaign && this.data.selectedCampaign.id === campaignId
        ? this.data.selectedCampaign
        : null;
      if (!campaign) {
        campaign = await api.acquisitionCampaign(campaignId, {
          user_id: user.id,
          scene: campaignScene(campaignId)
        });
      }
  ...
  async viewCampaign(event) {
    try {
      const user = await getApp().ensureUser();
      const campaignId = Number(event.currentTarget.dataset.id);
      const campaign = await api.acquisitionCampaign(campaignId, {
        user_id: user.id,
        scene: campaignScene(campaignId)
      });
      this.setData({
        ...
        selectedCampaignFormSchema: activityFormSchema(campaign),
        campaignFormValues: {}
      });
      this.startScreenHeartbeat(campaign);
  ...
  closeCampaignDetail() {
    this.stopScreenHeartbeat();
    this.setData({ selectedCampaign: null, invitePosterVisible: false, invitePoster: null });
  },
  ...
  startScreenHeartbeat(campaign) {
    ...
    const send = async () => {
      ...
      try {
        ...
        // Online status is best-effort; browsing should not be blocked by heartbeat failures.
      } finally {
  ...
  async ensureDefaultAddress(userId) {
    const addresses = await api.userAddresses(userId);
    ...
    if (!address) {
      wx.showModal({
        ...
        confirmText: "去编辑",
        success: res => {
          if (res.confirm) this.goAddressSettings();
        }
      });
      return null;
    }
  ...
  closeOrderFlow() {
    ...
    const flowCampaign = this.data.orderFlow.campaign;
    const selectedCampaign = this.data.selectedCampaign;
    const keepHeartbeat = flowCampaign && selectedCampaign && Number(flowCampaign.id) === Number(selectedCampaign.id);
    this.setData({ "orderFlow.visible": false });
    if (!keepHeartbeat) this.stopScreenHeartbeat();
  },
  ...
  async shareCampaign(event) {
    const campaignId = Number(event.currentTarget.dataset.id);
    try {
      this.setData({ invitePosterLoading: true });
      const user = await getApp().ensureUser();
      const poster = normalizeInvitePoster(await api.campaignInvitePoster(campaignId, user.id));
      wx.setStorageSync(campaignSceneKey(campaignId), poster.scene);
      this.setData({
        invitePoster: poster,
  ...
  onLoad(options = {}) {
    ...
    if (invite) {
      ...
      this.setData({ activeSection: "campaigns" });
      ...
      this.setData({ activeSection: "campaigns" });
      if (decodedScene) {
        wx.setStorageSync(campaignSceneKey(this.pendingCampaignId), decodedScene);
      }
  ...
  onShareAppMessage() {
    ...
    return {
      title: "必火次元",
      path: `/pages/home/index?scene=${userId}`
    };
  },
